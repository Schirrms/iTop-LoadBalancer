...
This Extension is highly inspired by the original 'ApplicationSolution' class. the idea is to have a versatile product, able to connect to virtually any Functional CI, as does the 'ApplicationSolution' class, but with some 'technical informations' available, mainly the IP address, not availavble in 'ApplicationSolution' and missing for our use.

## Further notes :

<ol>
	<li>As we use TeemIP, this extension has dependencies to 'TeemIP core'. It should not be a big work to remove this dependencies, but I don't have time to test it !</li>
...
This extension relies ont TeemIP, version >= 2.7

## State

Altough young, I consider this Extension in 'Stable' state. I don't think that major braking changes are expected.
...
Load Balancer are part of our general configuration, and need to be represented in our CMDB. iTop doens't have a native way to represent Load Balancer, but, until now, we did use the applicationSolution class to represent all our LoadBalancer configuration. The 'LoadBalancer' class is generic enough to represenet any king of dependencies between CI.

Unfortunately, this has (at least) two drawbacks :
 
 * Solution application have a very limited set of attributes, and miss at least IP address (witch is not a natural part of a Application 😊 ) 
 * ApplicationSolution are by design at a much higher level than LoadBalancer, so you will found technical stuff between functional process
...
A operational LoadBalancer configuration need 3 kind of resources :

 * An active LoadBalancer (preferably a kind of Cluster LoadBalancer, to avoid a SPOF in your infrastructure)
 * On the load Balancer, a 'vip' configuration : this is mainly the virtual IP Adress of the load balancing service, and also the port(s), protocols (mainly tcp or udp) if a layer 4 load balancer or the service(s) published (http/https, ftp ...).
 In the case of https, there is also the situation of the SSL terminaison. If the load Balancer 'terminates the session, it is also the load Balncer who would take care of the SSL Certificate(s).
 * On the load balancer 'Service Group' configuration : this is configuration known to the load balancer, who will take care of the incomming flow. You can alos see those service group as destination for the flow.

 In short, a load balancer configuration needs :
  * A vip : this is the point of connection for the traffic
	* one ore more service groups : those are the final destination of the traffic
 ...
 * Load Balancer to distribute the load and manage destination failures. In this case, the load banacer will be a 'Layer 4' Load Balancer, only redirecting the incoming IP flow to the different destinations, taking care of destination failure, but without any knowledge of the SSL contents.
 * SSL Offload : In this case, the SSL connection 'ends' on the load balancer, and the flow between the load balancer is in http (unencrypted). This is an easy way to centralizes SSL managment, and this reduce a little bit the load of the endpoint. I don't want to start here a security discussion, but, obviously, as a part of the traffic is unencrypted, security measure should be on place to be sure that traffic interception is not possible on this part of the flow.
 * SSL to SSL connection : In this case, the Load Balancer 'terminates' with his own certificate the client flow, and then initiates (an an SSL client) a connection to the actual endpoint. Usage case : distributes traffic over many endpoints, but this is also a way to 'hide' an application using a private SSL certificate.
 * unencrypted to SSL : Well, this is technically possible : connection unencrypted to the load balancer, and ssl connection from the load balancer to the endpoint(s). But I dont see a usage case!

## Walking trough an existing configuration

Suppose the following configuration : You have a pool of three VirtualMachine hosting a accounting web application. Those machines give the same service, and are here to absord the traffic. The service is published unencrypted on tcp port 40001.
...
In iTop, the Dependency can be seen as :

![iTop Load Balancer Dependency](img/LB-Dependency.png)

As you can see, I created 3 Laod balancer (roles) to represent this configuration:
...
And this is the view of the link to the underlying CIs (in this case three VirtualMachines, but it can be any FunctionalCIs from the Database)
![iTop Load Balancer VIP CIs List](img/LB-SG-Conf-CIs.png)
To be complete, I should also add a dependency to the Load Balancer cluster, as the Service Group cannot work without. But this complexifies the vue, and as, in case of a malfucnctionning cluster, the VIP would be red, the expected result is OK without this link.
...
And this is the view of the link to the underlying CIs (in this case two Generic Comm Device -another of my extensions-, but it can be any FunctionalCIs from the Database)
![iTop Load Balancer VIP CIs List](img/LB-SG-Conf-CIs.png)
